...
CREATE TABLE `sfm_order` (
  ...
  `products_amount` decimal COMMENT '商品总价, 单位为分, 1元为100',
  `credit_amount` DECIMAL COMMET '额度卡使用-100',
  `favour_amoount` decimal COMMET '优惠价格-100',
  `manual_amount` decimal COMMENT '人工干预-100',
  `pay_amount` decimal COMMENT '支付价格 pay_amount=ship_amount + products_amount + credit_amount + favour_amoount + manual_amount',
  ...
  `status` tinyint(4) COMMENT '0表示删除',
